  ...
  if (counters) {
    counters.forEach(counter => {
      ...
      decBtn.addEventListener('click', (e) => {
        if (computedCount.textContent > 0) {
          computedCount.textContent--
        }

        const childrenChangeableList = e.target.closest('.guests-modal__rooms-list-item').querySelector('.guests-modal__children-list')

        if (childrenChangeableList.children.length > 0) {
          const lastChild = childrenChangeableList.children[childrenChangeableList.children.length - 1]
          lastChild.remove()
        }
      ...
      incBtn.addEventListener('click', (e) => {
        ...
        if (childTemplateId) {
          let templateFragment = document.querySelector(`#${childTemplateId}`).content
          let templateElement = templateFragment.firstElementChild.cloneNode(true)
          const targetChangeableList = e.target.closest('.guests-modal__rooms-list-item').querySelector('.guests-modal__children-list')
          targetChangeableList.appendChild(templateElement)

          const changeAmount = templateElement.querySelector('.changeable-amount')
...
if (guestsInput) {
  ...
  body.addEventListener('click', (e) => {
    ...
    const isModalClick = e.target.classList.contains('guests-modal') || e.target.classList.contains('guests-modal__delete-btn') || e.target.closest('.guests-modal') || e.target === guestsInput
...
if (guestsModal) {
  ...
  completeBtn.addEventListener('click', (e) => {
    ...
    const adultsCounts = guestsModal.querySelectorAll('.main-counter._adults .main-counter__count')
    const childrenCounts = guestsModal.querySelectorAll('.main-counter._children .main-counter__count')
    ...
    guestsInput.value = `${adultsNum} ${adultsNum === 1 ? 'взрослый' : 'взрослых'} , ${childrenNum} ${childrenNum === 1 ? 'ребенок' : 'детей'}`
